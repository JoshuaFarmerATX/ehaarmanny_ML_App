...
number_of_days = st.sidebar.number_input(
    "Predict Number of Days:", min_value=1, max_value=30, value=1, step=1
)
...
def return_raw_data(region):
    ...
    if region_selector == "USA":
        df = pd.read_json(
            f"https://api-pc6dbtrtla-uc.a.run.app/API/timeseries/{region}"
        )
    else:
        state_str = region.replace(" ", "%20")
        df = pd.read_json(
            f"https://api-pc6dbtrtla-uc.a.run.app/API/us/timeseries/totals/{state_str}"
        )
    return df
...
def return_model_data(region, days, cases_or_deaths_str):
    ...
    if region_selector == "USA":
        df = pd.read_json(
            f"https://models-pc6dbtrtla-uc.a.run.app/{cases_or_deaths_str}/{days}/"
        )  # need number of days
    else:
        state_str = region.replace(" ", "%20")
        df = pd.read_json(
            f"https://models-pc6dbtrtla-uc.a.run.app/states/{cases_or_deaths_str}/{state_str}/{days}/"
        )  # need number of days
    return df
...
st.title(
    f"COVID 19 Projected Number of {cases_or_deaths} in {region_selector} for the Next {number_of_days} Day(s)"
)
...
if region_selector == "USA":
    st.table(
        df[["Country", "Totals as of Date", "Cases", "Deaths"]].sort_values(
            by="Totals as of Date", ascending=False
        )
    )
else:
    st.table(
        df[["State", "Totals as of Date", "Cases", "Deaths"]].sort_values(
            by="Totals as of Date", ascending=False
        )
    )
