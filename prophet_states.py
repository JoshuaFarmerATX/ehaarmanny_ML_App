...
def prepdata_cases(state):
    ...
    df = pd.read_json(
        f"http://api-pc6dbtrtla-uc.a.run.app/API/us/timeseries/totals/{state_str}"
    )
    ...
    df_cases = df_cases[["Date", "NewCases"]].rename(
...
def prepdata_deaths(state):
    ...
    df_deaths = df_deaths[["Date", "NewDeaths"]].rename(
...
def predict_cases(state, days):
    ...
    df_forecast = (
        df_forecast.assign(yhat=lambda df: df["yhat"].apply(lambda e: max(0, e)))
        .assign(yhat_lower=lambda df: df["yhat_lower"].apply(lambda e: max(0, e)))
        .assign(yhat_upper=lambda df: df["yhat_upper"].apply(lambda e: max(0, e)))
    )
